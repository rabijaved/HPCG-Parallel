...
//#endif
  ...
  CopyVector(x, p);
  TICK(); ComputeSPMV(A, p, Ap); TOCK(t3); // Ap = A*pe
  ...
  for (int k=1; k<=max_iter && normr/normr0 > tolerance; k++ ) {
    ...
    if (A.geom->rank==0 && (k%print_freq == 0 || k == max_iter))
      HPCG_fout << "Iteration = "<< k << "   Scaled Residual = "<<normr/normr0 << "  normr = " << normr << "  normr0 = " << normr0 << std::endl;
#endif
    niters = k;
  }
  ...
  printf("ComputeMG %f\n", t5);
  printf("ComputeSPMV %f\n", t3);
  printf("ComputeWAXPBY %f\n", t2);
  printf("Dotproduct %f\n", t1);
