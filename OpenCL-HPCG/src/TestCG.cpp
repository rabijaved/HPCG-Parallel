...
extern cl_mem spmv_mtxIndL_buff, spmv_matrixValues_buff;
...
int TestCG(SparseMatrix & A, CGData & data, Vector & b, Vector & x, TestCGData & testcg_data) {
  ...
  FlattenMatrix(A,  numberOfNonzerosPerRow);  //fill flat matrix

  err |= clEnqueueWriteBuffer(queue, spmv_mtxIndL_buff, CL_FALSE,
    0,  sizeof(int)*A.localNumberOfRows*numberOfNonzerosPerRow, A.flat_mtxIndL, 0, NULL, NULL);


  err |= clEnqueueWriteBuffer(queue, spmv_matrixValues_buff, CL_FALSE,
    0, sizeof(double)*A.localNumberOfRows*numberOfNonzerosPerRow, A.flat_matrixValues, 0, NULL, NULL);
